...
// DBEntry stores info for one SQLite database
type DBEntry struct {
...
func main() {
	...
	// 扫描 pjs_data 目录，自动加载所有 .db 文件
	err := scanDatabases("pjs_data")
	if err != nil {
		log.Printf("Warning: failed to scan pjs_data: %v", err)
	}

	// 创建或打开 user-imported database
...
// scanDatabases 扫描指定目录下的所有 .db 文件并建立连接
func scanDatabases(dir string) error {
	...
	for _, entry := range entries {
		...
		displayName := name
		if strings.HasPrefix(name, "tz-") {
			displayName = "台州中院 " + strings.TrimPrefix(name, "tz-")
		} else if strings.HasPrefix(name, "wl-") {
			displayName = "温岭法院 " + strings.TrimPrefix(name, "wl-")
		}
		...
		dbEntries[displayName] = &DBEntry{
			Name: displayName,
			Path: path,
			DB:   db,
		}
		log.Printf("Loaded database: %s -> %s", displayName, path)
	}

	return nil
}

// getDatabases 返回当前可用的数据库列表
func getDatabases(c *gin.Context) {
	var names []string
	for name := range dbEntries {
		names = append(names, name)
	}
	// 用户导入数据库始终放在最后
	names = append(names, "已导入数据")
...
func searchDocuments(c *gin.Context) {
	...
	sourceDB := c.Query("source")   // 显示名称 或 "全部" / "已导入数据"
	...
	if sourceDB == "全部" {
		for name, entry := range dbEntries {
			var dbResults []Document
			dbResults, err = searchInDB(entry.DB, keywords, excludeKeywords, searchType, docType, name)
			if err != nil {
				log.Printf("Error searching in %s database: %v", name, err)
				continue
			}
	...
	} else if sourceDB == "已导入数据" {
		results, err = searchInDB(userDB, keywords, excludeKeywords, searchType, docType, "已导入数据")
		if err != nil {
			log.Printf("Error searching in 已导入数据 database: %v", err)
		...
		entry, ok := dbEntries[sourceDB]
		if !ok {
			c.JSON(http.StatusBadRequest, gin.H{"error": "未知的数据源: " + sourceDB})
			return
		}
		results, err = searchInDB(entry.DB, keywords, excludeKeywords, searchType, docType, sourceDB)
		if err != nil {
			log.Printf("Error searching in %s database: %v", sourceDB, err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "数据库搜索出错: " + err.Error()})
			return
		}
	}
